...
class TuyaDevice {
    ...
    publishDpsTopics() {
        try {
            ...
            debugState('Publishing MQTT DPS JSON: ' + dpsStateTopic + ' -> ', data)
            ...
            for (let key in this.dps) {
                ...
                if (this.dps[key].updated) {
                    ...
                    debugState('Publishing MQTT DPS'+key+': '+dpsKeyTopic+' -> ', data)
	...
	//correct syntax:
        ...
        //this.publishMqtt(this.options.baseTopic+'status', status)
        await utils.sleep(1)
        this.init()
    }
	...
	//add qos and retain in debug message
        if (isDebug) { debugState(topic, message, _qos, _retain) }
